...
def captcha2text(image_list, height=CAPTCHA_HEIGHT, width=CAPTCHA_WIDTH):
    ...
    with tf.Session() as sess:
        ...
        #saver.restore(sess, 'D:\\Samples\\Projects\\captchaCnn\\model\\0.96captcha.model-38300')
...
if __name__ == '__main__':
    ...
    pre_text = captcha2text(imageList)
    right_num = 0
    for i in range(TEST_NUM):
        if(textList[i]==pre_text[i]):
            right_num += 1
        print('Label:', textList[i], ' Predict:', pre_text[i])
